...
<script>
            function autoriradosview()
            {
               window.location.href = "./Rest_Autorizado/ViewAut.php";
            }
</script>
<script>
...
<div class="load-animate animated fadeInUp">
			...
			<form action="arrayfech.php" method="post" name="busqueda" class="" role="form" novalidate> 
                <br>
...
<!--//COLOCAR AQUI LOS FILTROS -->
            <div class="col-xs-6 col-sm-3 col-md-3 col-lg-3 pull-left">
            <div>
    <select name="te" id="empleado_sucursal" class="form-control" data-live-search="true" class="form-control">
            ...
            <div class="col-xs-6 col-sm-3 col-md-3 col-lg-3 pull-right">
							...
							<br><button class="btn btn-danger btn-lg btn-block" id= "btn-search" name="btn_search" type="submit"> <h5><span class="glyphicon glyphicon-repeat"></span> ACTUALIZAR</h5></button>
													...
													<div class="form-group">
														...
														<input tabindex="1" aria-describedby="basic-addon1" id="filtrar" value="" type="text" class="form-control" name="te" placeholder="Busqueda">
                    ...
                    <tbody class="buscar">
                            ...
                            $resultado=mysqli_query( $conexion, $query ) or die ( "No se pueden mostrar los canales");
                                              ...
                                              if ($_SESSION['Modulo']== 5){
                                                	//mysqli_close($conexion);
														mysqli_next_result($conexion); 
                                            $query="SELECT E.*, INV.* FROM tmk_detalle_inventarios E INNER JOIN tmk_inventarios INV ON
                                            E.id_interno_inventario	= INV.id_interno_inventario"; //CAMBIAR POR VISTA 
                                            $resultado=mysqli_query( $conexion, $query ) or die ( "No se pueden mostrar los canales");
                                              }   
                                                }
                                                ...
                                                while ($row=mysqli_fetch_array($resultado))
                                                    {
                                                        echo '<tr>';
                                                        $idenvi=$row['id_interno_inventario'];
                                                    echo '<td> '; echo $row['id_inventario'];echo '</td>';
                                                    
                                                    echo '<td> '; echo $row['id_interno_inventario'];echo '</td>';
                                                    echo '<td> '; echo $row['codigo_empleado'];echo '</td>';
                                                    ...
                                                    ?> <td>  <a class="btn btn-success" href='vernota.php?id=<?php echo $idenvi;?>&com=<?php echo $i ?>' ><i class="fa fa-edit"></i> Editar</a> <?php echo '</td>';
                                                    ?> <?php
...
(function ($) {
  ...
  $('#te').on('change', function () {
    ...
    var rex = new RegExp(selectedOption, 'i');
