...
include('../../include/config.inc');
    ...
    $id_autorizado= $_SESSION['id_autorizado']; 
...
<div class="topnav" id="myTopnav" >
 ...
 $modulo_aut='<a href="../../AppCode/Rest_Autorizado/ViewAut.php"><i class="fa fa-book" aria-hidden="true"></i> Modulo de Autorizaciones</a>'; 
 $reportes='<a class="active"  href="../Report_master.php"><i class="fa fa-line-chart" aria-hidden="true"></i> Reportes</a>';
...
<div class="load-animate animated fadeInUp">
			...
			<center><h1>REPORTES DE NOTAS AUTORIZADAS IMPRESION</h1></center>
			</div>		    		
			</div>
			...
			<form action="arrayfechautpdfbodegaimpresa.php" method="post" name="busqueda" class="" role="form" novalidate> 
                <br>
...
<div class="panel-body">
         ...
         <br> 
            ...
            <select name="autorizadores" id="autorizadores"  class="form-control">
									<option value="0">Seleccione Informe de autorizacion bodega</option>
				 
									<?php
										$query="Select*from usuarios where id_rol = '7' or id_rol = '9';";
										...
										<option 
												...
												value="<?php echo $row['codigo_empleado']; ?>"    > <?php   echo $row['first_name']; ?> </option>
												<?php  $i++;
									} ?>
                                        ...
                                        <?php
                                                                  ...
                                                                  if (isset($_SESSION['min'])){
                                                                      if (isset($_SESSION['max'])){
                                                                        if (isset($_SESSION['autorizadores'])){
                                                                          if ($_SESSION['flex']=true){
                                                                          
                                                                          $min = $_SESSION['min'];
                                                                          $max =$_SESSION['max'];
                                                                          $autorizadores = $_SESSION['autorizadores'];
                                                                          $query="call SP_REPORTS_LLENAR_TABLA_FORMULARIO_IMPRESAS_BODEGA('$min','$max','$autorizadores')";
                                                  $resultado=mysqli_query( $conexion, $query ) or die ( "No se pueden mostrar los canales");
                                                ...
                                                while ($row=mysqli_fetch_array($resultado))
                                                    ...
                                                    if ($row['id_estado']=1){
                                                        echo '<td> '; echo "Autorizada";echo '</td>';
                                                                                }
                                                    
                                                    ?> <td>  <a class="btn btn-success" href='fragmentvisorimpresa.php?id=<?php echo $idenvi;?>&com=<?php echo $i ?>' ><i class="fa fa-eye"></i> Ver</a> <?php echo '</td>';
                                                    ?> <?php
